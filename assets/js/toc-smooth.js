...
function initScrollSpy() {
  ...
  function updateNestedRoundedCorners(currentId) {
    ...
    allLinks.forEach(link => {
      link.classList.remove('rounded-t', 'rounded-b');
    });
    ...
    // 应用圆角
    if (isFirst) {
      currentLink.classList.add('rounded-t');
    }
    if (isLast) {
      currentLink.classList.add('rounded-b');
    }
  }
